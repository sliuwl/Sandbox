...
def parse_axsf(filepath):
    ...
    if animsteps is None or position_start is None:
    ...
    return animsteps, primvec, modes


def write_mode_xsf(output_path, animsteps, primvec, mode_lines):
    """Write a single mode to an XSF file."""
...
def main():
    ...
    animsteps, primvec, modes = parse_axsf(input_file)
    ...
    for idx, mode_lines in enumerate(modes, start=1):
        ...
        write_mode_xsf(output_file, animsteps, primvec, mode_lines)
